...
{
    public partial class OrgDep : UI.WebPage
    {
        ...
        private void BindData(bool isFirstPage)
        {
            ...
            int pageSize = 1;
            //分页查询语句
            string sql = "select row_number() over(order by  Addtime desc,EmplId DESC) as rownum,* FROM OrgEmployees where Status=1 ";
            ...
            //分页方法
            Pager1.GetDataBind("Repeater", "rptList", sql, pageNum, pageSize, "", "rownum", "OrgDep.aspx?id=" + Request.QueryString["id"] + "&name=" + name + "&");
            #endregion
        }
