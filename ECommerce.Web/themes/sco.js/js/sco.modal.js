	...
	$.extend(Modal.prototype, {
		show: function() {
			...
			if (this.options.remote !== undefined && this.options.remote != '' && this.options.remote !== '#') {
				...
				this.$modal.find('.inner').load(this.options.remote,'f' + (Math.random()*1000000), function() {
					if (spinner) {
						spinner.stop();
					}
		...
		, close: function () {
		    try {
		        document.getElementById('UploadAttachments1').cancelall();
		    } catch (e) {
		    }
		    try {
		        document.getElementById('UploadAttachments2').cancelall();
		    } catch (e) {
		    }
		    try {
		        document.getElementById('UploadAttachments3').cancelall();
		    } catch (e) {
		    }
